...
- [ ] MCP server with 15 Exchange management tools (mailbox, DAG, mail flow, security, hybrid)
- [ ] Python chat application with Azure AD/Entra ID SSO authentication
- [ ] Pass-through user identity — Exchange sees the actual colleague's identity via Kerberos delegation
- [ ] Polished internal tool UI with conversation history persisting across sessions
- [ ] Multiple conversation threads with sidebar navigation
- [ ] Tool visibility — show which Exchange tool was invoked and what data was returned
- [ ] Export/share — copy or export responses for reports or tickets
- [ ] Azure OpenAI integration via MMC corporate endpoint (gpt-4o-mini-128k)
- [ ] Async PowerShell execution layer with per-call PSSession lifecycle
- [ ] DNS-based security lookups (DMARC/SPF/DKIM) via dnspython
- [ ] All operations read-only in v1
...
- **Tech stack:** Python 3.11+ (MCP server + chat app), PowerShell 5.1+ (Exchange cmdlets), Flask/FastAPI + Jinja2 (frontend)
...
- **Authentication:** Azure AD/Entra ID SSO for chat app, Kerberos Constrained Delegation for pass-through identity to Exchange

## Key Decisions

| Decision | Rationale | Outcome |
|----------|-----------|---------|
| Python full-stack (Flask/FastAPI + Jinja2) over React SPA | Single language across MCP server and chat app, simpler deployment on Windows | — Pending |
| Pass-through user identity over service account | Per-user audit trail, respects existing Exchange RBAC, colleagues see only what they're authorized to see | — Pending |
| Azure AD/Entra ID SSO | Corporate identity standard at MMC, required for pass-through identity chain | — Pending |
| All 15 tools in v1 | Tools are well-scoped and documented, incremental delivery doesn't reduce complexity significantly | — Pending |
| Per-call PSSession (no pooling) | Eliminates session leak risk, fresh auth per call, simpler implementation for demo | — Pending |
