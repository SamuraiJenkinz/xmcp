...
This roadmap delivered a complete Exchange management system for Marsh McLennan: an MCP server exposing 15 Exchange infrastructure tools, paired with a Python chat application backed by Azure AD SSO and Azure OpenAI. The delivery followed a strict bottom-up dependency chain — Exchange client first (every tool depends on it), then the MCP protocol layer, then all 15 tools across three phases, then the chat orchestration layer, then the UI. Each phase produced a verifiable capability before the next phase began.
...
**Goal**: All hybrid tools are implemented and validate the live Exchange Online connector health — completing the full 15-tool MCP server
**Depends on**: Phase 5
...
**Success Criteria** (what must be TRUE):
  ...
  2. get_connector_status reports hybrid connector health with a live test result against the Exchange Online endpoint
  3. All 15 tools enumerate correctly when queried via the MCP inspector — the server is fully complete
**Plans**: 2 plans
...
**Note:** get_migration_batches (HYBR-02) removed — MMC does not use migration batches. Tool count finalized at 15 (14 Exchange + ping).
...
**Key Decisions:**
- Flask 3.x + Waitress over FastAPI — run async PowerShell via run_in_executor
- Per-call PSSession, no pooling — accept 2-4s latency for simplicity
- SQLite for conversation persistence — zero ops, correct for <100 concurrent users
- Official mcp SDK (not fastmcp) — stdio transport for v1
- Interactive auth default, CBA optional — user-requested dev workflow
- -EncodedCommand over -Command — prevents cp1252 corruption
- openai.OpenAI (not AzureOpenAI) — MMC gateway URL format incompatible with Azure SDK
- get_migration_batches removed — MMC does not use migration batches

**Issues Resolved:**
- SSE field name mismatch (event.content vs event.delta) fixed in commit 27bd574
- Tool events (params/result) wired end-to-end from openai_client through chat.py SSE to app.js panels

**Technical Debt Incurred:**
- Tool events not persisted to SQLite — historical messages lose tool panels on thread reload
- Copy button not rendered on historical messages — only live-streamed messages
- CHATGPT_ENDPOINT not in secrets.py pipeline — bare env var required
- Stale module docstring in tools.py
